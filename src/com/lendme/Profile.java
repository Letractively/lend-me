...
public class Profile {
	...
	protected Collection<Lending> getLendingRecords(String kind) throws Exception{
		...
		if ( kind.equals("emprestador") ){
			...
			result.addAll(owner.getUserOperationManager().getItemManager().getLentRegistryHistory());
			result.addAll(owner.getUserOperationManager().getItemManager().getMyLentItems());
			return result;
		}
		else if ( kind.equals("beneficiado") ){
			List<Lending> result = new ArrayList<Lending>();
			result.addAll(owner.getUserOperationManager().getItemManager().getBorrowedRegistryHistory());
			result.addAll(owner.getUserOperationManager().getItemManager().getMyBorrowedItems());
			return result;
		}
